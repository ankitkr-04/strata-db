...
class WalStaging {
    public:
    WalStaging(memory::EpochManager& epoch_manager, memory::Arena& staging_arena) noexcept : epoch_manager_(epoch_manager), staging_arena_(staging_arena) {}
    ...
    //Cold Path
    // Called by internal WAL Orchestrator thread to retrieve the staged record for flushing to disk. This is a destructive read and will clear the staging area after reading.
