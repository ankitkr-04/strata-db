...
struct WalConfig {
    ...
    // Hardware AWUPF (Atomic Write Unit Power Fail) is typically 4 KiB, so we
    // align WAL blocks to that boundary for maximum durability.
    static constexpr std::size_t DEFAULT_WAL_BLOCK_SIZE = 4_KiB;

    // How much data WAL can stage in memory before forcing a hard disk flush.
    ...
    // 0 = Auto-Detect via OS probing. > 0 forces a specific queue depth.
