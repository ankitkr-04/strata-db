...
// How many bytes of node allocation we target per thread before hitting
// kMinimumOpsPerThread. Can be overridden by STRATADB_BENCH_OPS_PER_THREAD.
constexpr std::size_t kPerThreadTargetBytes = 4ULL * 1024ULL * 1024ULL;
constexpr std::size_t kMinimumOpsPerThread = 4096;

// Default extra arena budget above the computed minimum.
// Override with STRATADB_BENCH_ARENA_HEADROOM_MIB.
constexpr std::size_t kDefaultHeadroom = 16ULL << 20; // 16 MiB
...
// Cache-line isolated per-thread stats.
//
// alignas(CACHE_LINE_SIZE) guarantees the struct is placed at a 64-byte
// boundary. The C++ standard requires sizeof to be a multiple of the
// alignment for any aligned struct, so the compiler inserts trailing padding
// automatically. No hand-rolled _pad[] arithmetic needed — that approach is
// fragile if field sizes change.
struct alignas(utils::CACHE_LINE_SIZE) WorkerStats {
    std::vector<void*> allocs;
    std::size_t ops_completed{0};
    bool failed{false};
    // Compiler inserts ~31 bytes of trailing padding to reach 64 bytes.
};
static_assert(sizeof(WorkerStats) % utils::CACHE_LINE_SIZE == 0,
              "WorkerStats risks false sharing — struct must be a cache-line multiple.");

// Returns ops-per-thread for this profile.
// STRATADB_BENCH_OPS_PER_THREAD overrides the default size-derived calculation.
[[nodiscard]] auto ops_for_profile(const NodeProfile& profile) noexcept -> std::size_t {
    const std::size_t override_val = global_bench_config().ops_per_thread;
    if (override_val != 0)
        return override_val;
...
void run_benchmark(benchmark::State& state) {
    ...
    const std::size_t per_thread_ops = ops_for_profile(profile);
    ...
    // Resize (not reserve) to allow branchless `allocs[op] = mem` in the hot loop.
    for (std::size_t t = 0; t < tc; ++t) {
    ...
    // Three-barrier sandwich: setup → go → done.
    // Each provides a happens-before guarantee without spinning.
    ...
    for (int ti = 0; ti < thread_count; ++ti) {
        workers.emplace_back([&, t = static_cast<std::size_t>(ti)]() {
            ...
            while (true) {
                ...
                // Local registers — avoid cache line writes inside the hot loop.
                ...
                // write-back (once, outside timed window)
    ...
    for (auto _ : state) {
        ...
        if constexpr (Mode != AllocationMode::BaselineNew) {
            // Capacity = all nodes + TLAB slabs + configurable headroom.
            const std::size_t capacity = (per_thread_ops * tc * node_size)
        ...
        shared_arena = arena.get(); // visible to workers via barrier below
    ...
    state.PauseTiming();
    stop_flag = true;
    setup_barrier.arrive_and_wait(); // final release for clean thread exit
